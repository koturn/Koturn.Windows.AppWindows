...
using Koturn.Windows.AppWindows.Internals;
...
namespace Koturn.Windows.AppWindows
{
...
#endif  // SUPPORT_LIBRARY_IMPORT
    {
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.MoveWindow(IntPtr, int, int, int, int, bool)"/> is failed.</exception>
        public static void MoveWindow(IntPtr hWnd, int x, int y, int width, int height, bool doRepaint)
        {
            if (!SafeNativeMethods.MoveWindow(hWnd, x, y, width, height, doRepaint))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.MoveWindow) + " failed");
            }
        }
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.GetWindowRect"/>
        /// or <see cref="SafeNativeMethods.SetWindowPos"/> is failed.</exception>
        public static void SetWindowSize(IntPtr hWnd, int width, int height, bool doCenterResize = true, bool doActivate = true)
        {
            ...
            if (!SafeNativeMethods.SetWindowPos(hWnd, IntPtr.Zero, x, y, width, height, flags))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.SetWindowPos) + " failed");
            }
        }
        ...
        /// <param name="doActivate">Activate window on resize or not.</param>
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.GetWindowRect"/>,
        /// <see cref="SafeNativeMethods.GetClientRect"/> or <see cref="SafeNativeMethods.SetWindowPos"/> is failed.</exception>
        public static void SetClientSize(IntPtr hWnd, int width, int height, bool doCenterResize = true, bool doActivate = true)
        {
            ...
            if (!SafeNativeMethods.SetWindowPos(hWnd, IntPtr.Zero, x, y, windowWidth, windowHeight, flags))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.SetWindowPos) + " failed");
            }
        }
        ...
        /// </summary>
        /// <param name="hWnd">A handle to the window.</param>
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.ShowWindow(IntPtr, CmdShow)"/> is failed.</exception>
        public static void Maximize(IntPtr hWnd)
        {
            if (!SafeNativeMethods.ShowWindow(hWnd, CmdShow.ShowMaximized))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.ShowWindow) + " failed");
            }
        }
        ...
        {
            if (!SafeNativeMethods.ShowWindow(hWnd, CmdShow.ShowMinimized))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.ShowWindow) + " failed");
            }
        }
        ...
        /// </summary>
        /// <param name="hWnd">A handle to the window.</param>
        /// <param name="style">Combination of <see cref="WindowStyleFlags"/> values.</param>
        /// <exception cref="Win32Exception">Thrown when ShowWindow, Win32 API function is failed.</exception>
        public static nuint SetWindow(IntPtr hWnd, nuint style)
        {
            var oldStyle = SafeNativeMethods.SetWindowLongPtr(hWnd, (int)GwlIndice.Style, style);
            if (oldStyle == 0)
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.SetWindowLongPtr) + " failed");
            }
            return oldStyle;
        }
        ...
        /// <exception cref="Win32Exception">Thrown when SetForegroundWindow, Win32 API function is failed.</exception>
        public static void SetForegroundWindow(IntPtr hWnd)
        {
            if (!SafeNativeMethods.SetForegroundWindow(hWnd))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.SetForegroundWindow) + " failed");
            }
        }
        ...
        /// </summary>
        /// <param name="hWnd">A handle to the window.</param>
        /// <param name="hPrevMenu">Previous menu.</param>
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.ShowWindow(IntPtr, CmdShow)"/> is failed.</exception>
        ...
        {
            if (!SafeNativeMethods.ShowWindow(hWnd, CmdShow.Restore))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.ShowWindow) + " failed");
            }
        }
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.GetWindowRect"/> is failed.</exception>
        public static WindowRect GetWindowRect(IntPtr hWnd)
        {
            if (!SafeNativeMethods.GetWindowRect(hWnd, out var windowRect))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.GetWindowRect) + " failed");
            }
            return windowRect;
        }
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.GetClientRect"/> is failed.</exception>
        public static WindowRect GetClientRect(IntPtr hWnd)
        {
            if (!SafeNativeMethods.GetClientRect(hWnd, out var clientRect))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.GetClientRect) + " failed");
            }
            return clientRect;
        }
        ...
        {
            return SafeNativeMethods.GetMenu(hWnd);
        }
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.GetMenu(IntPtr)"/> is failed.</exception>
        public static void SetMenu(IntPtr hWnd, IntPtr hMenu)
        {
            if (!SafeNativeMethods.SetMenu(hWnd, hMenu))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.SetMenu) + " failed");
            }
        }
        ...
        /// <exception cref="Win32Exception">Thrown then <see cref="SafeNativeMethods.RemoveMenu(IntPtr, uint, MenuFlags)"/> failed.</exception>
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.MonitorFromWindow(IntPtr, MonitorDefaultFlags)"/> is failed.</exception>
        public static IntPtr MonitorFromWindow(IntPtr hWnd, MonitorDefaultFlags flag = MonitorDefaultFlags.ToNearest)
        {
            var hMonitor = SafeNativeMethods.MonitorFromWindow(hWnd, flag);
            if (hMonitor == IntPtr.Zero)
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.MonitorFromWindow) + " failed");
            }
            return hMonitor;
        }
        ...
        /// <exception cref="Win32Exception">Thrown when <see cref="SafeNativeMethods.GetMonitorInfo(IntPtr, ref MonitorInfo)"/> is failed.</exception>
        public static MonitorInfo GetMonitorInfo(IntPtr hMonitor)
        {
            var monitorInfo = MonitorInfo.Create();
            if (!SafeNativeMethods.GetMonitorInfo(hMonitor, ref monitorInfo))
            {
                ThrowHelper.ThrowLastWin32Exception(nameof(SafeNativeMethods.GetMonitorInfo) + " failed");
            }
            return monitorInfo;
        }
...
#endif  // SUPPORT_LIBRARY_IMPORT
        {
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(SetWindowPos), SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool SetWindowPos(IntPtr hWnd, IntPtr hWndInsertAfter, int x, int y, int cx, int cy, SwpFlags flags);
#else
            [DllImport("user32.dll", EntryPoint = nameof(SetWindowPos), ExactSpelling = true, SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(GetWindowRect), SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool GetWindowRect(IntPtr hWnd, out WindowRect rect);
#else
            [DllImport("user32.dll", EntryPoint = nameof(GetWindowRect), ExactSpelling = true, SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(GetClientRect), SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool GetClientRect(IntPtr hWnd, out WindowRect rect);
#else
            [DllImport("user32.dll", EntryPoint = nameof(GetClientRect), ExactSpelling = true, SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(SetForegroundWindow), SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool SetForegroundWindow(IntPtr hWnd);
#else
            [DllImport("user32.dll", EntryPoint = nameof(SetForegroundWindow), ExactSpelling = true, SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(MoveWindow), SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool MoveWindow(IntPtr hWnd, int x, int y, int width, int height, [MarshalAs(UnmanagedType.U1)] bool doRepaint);
#else
            [DllImport("user32.dll", EntryPoint = nameof(MoveWindow), ExactSpelling = true, SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static extern bool MoveWindow(IntPtr hWnd, int x, int y, int width, int height, [MarshalAs(UnmanagedType.U1)] bool doRepaint);
#endif  // SUPPORT_LIBRARY_IMPORT
            ...
            /// To determine success or failure, clear the last error information by calling SetLastError with 0, then call <see cref="SetWindowLongPtr(IntPtr, int, nuint)"/>.
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(SetWindowLongPtr) + "W", SetLastError = true)]
            public static partial nuint SetWindowLongPtr(IntPtr hWnd, int nIndex, nuint dwNewLong);
#else
            [DllImport("user32.dll", EntryPoint = nameof(SetWindowLongPtr) + "W", ExactSpelling = true, CharSet = CharSet.Unicode, SetLastError = true)]
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(GetMenu), SetLastError = true)]
            public static partial IntPtr GetMenu(IntPtr hWnd);
#else
            [DllImport("user32.dll", EntryPoint = nameof(GetMenu), ExactSpelling = true, SetLastError = true)]
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(SetMenu), SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool SetMenu(IntPtr hWnd, IntPtr hMenu);
#else
            [DllImport("user32.dll", EntryPoint = nameof(SetMenu), ExactSpelling = true, SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            ...
            /// <summary>
            /// The <see cref="MonitorFromWindow(IntPtr, MonitorDefaultFlags)"/> function retrieves a handle
            ...
            /// <para>If the window is currently minimized, <see cref="MonitorFromWindow(IntPtr, MonitorDefaultFlags)"/> uses
            /// the rectangle of the window before it was minimized.</para>
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(MonitorFromWindow), SetLastError = true)]
            public static partial IntPtr MonitorFromWindow(IntPtr hWnd, MonitorDefaultFlags flag);
#else
            [DllImport("user32.dll", EntryPoint = nameof(MonitorFromWindow), ExactSpelling = true, SetLastError = true)]
            public static extern IntPtr MonitorFromWindow(IntPtr hWnd, MonitorDefaultFlags flag);
#endif  // SUPPORT_LIBRARY_IMPORT

            /// <summary>
            /// The <see cref="GetMonitorInfo(IntPtr, ref MonitorInfo)"/> function retrieves information about a display monitor.
            /// </summary>
            /// <param name="hMonitor">A handle to the display monitor of interest.</param>
            /// <param name="monitorInfo">
            /// <para>A reference to a <see cref="MonitorInfo"/> or MONITORINFOEX structure that receives information about the specified display monitor.</para>
            /// <para>You must set the cbSize member of the structure to sizeof(MONITORINFO) or sizeof(MONITORINFOEX)
            /// before calling the <see cref="GetMonitorInfo(IntPtr, ref MonitorInfo)"/> function.
            ...
            /// </remarks>
#if SUPPORT_LIBRARY_IMPORT
            [LibraryImport("user32.dll", EntryPoint = nameof(GetMonitorInfo) + "W", SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            public static partial bool GetMonitorInfo(IntPtr hMonitor, ref MonitorInfo monitorInfo);
#else
